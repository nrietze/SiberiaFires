...
training_polygons <- vect('./data/geodata/feature_layers/training_polygons/training_polygons_burn_area.shp') %>% 
  project('EPSG:32655') %>% 
  filter((LandCover != 'mixed') | (is.na(LandCover))) # remove the mixed training polygons from training data
...
get_raster_values <- function(fn_raster, poly_mask, features, sel_bands){
  ...
  sel_features <- features %>% 
    ...
    mutate(.,ID = 1:nrow(.)) %>% 
    mutate(iswater = factor(if_else(is.na(LandCover), 'no_water','water'),
                            labels = c('no_water','water') ) 
           )
  ...
  raster_focal_vals <- terra::extract(rast_focal, 
                                      sel_features)[,c('ID','gcc_sd','rcc_sd','bcc_sd','ndwi_sd','ndvi_sd')]
...
predict_water_area <- function(aoi_name, raster_files_pre, raster_files_post, training_polygons, poly_mask){
  ### 1. configure inputs ----
  ...
  calculate_separability <- function(variable_name) {
    separability(
      filter(training_sample, iswater == 'no_water') %>% ungroup() %>% select(!!variable_name ),
      filter(training_sample, iswater == 'water') %>% ungroup() %>% select(!!variable_name )
    ) %>%
  ...
  fn_conf_matrix <- paste0("tables/rf_confusion_matrix_water_",site,"_",pred_choice,".txt")
...
if(predict_all_aois){
  ...
  clusterExport(cl, c("predict_water_area","get_raster_values"), 
                envir=environment())
  ...
  predict_water_area(aoi_names, raster_files_pre, raster_files_post, training_polygons, poly_mask)
}
